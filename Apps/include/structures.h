...
class ThermoCreator: public BioFW::IDatabaseCreator<FaceRecord> {
	public:
		...
		virtual Db createDatabase() {
			...
			addFolder(db, basePath + "fitClean/");
			addFolder(db, basePath + "notredame/");
			addFolder(db, basePath + "germany/");
			addFolder(db, basePath + "equinox/");

			return db;
		}

		virtual Db createPureDatabase() {
					Db db;
		...
		void addFolder(Db & db, std::string path){
			...
			for (std::set<std::string>::iterator i = records.begin(); i != records.end(); i++) {
				...
				BioFW::BiometricID bid(st[0].substr(basePath.length()), "Face");
